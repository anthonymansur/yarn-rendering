...
{
	if (p1.posWarp != nullptr)
	{
		*p2 = p1.posWarp;
		if ((*p2)->posWeft != nullptr)
		{
			*p3 = (*p2)->posWeft;
			if ((*p3)->negWarp != nullptr)
			{
				*p4 = (*p3)->negWarp;
			}
			else
...
std::vector<Strand> Pattern2::fabricTraversal(FabricVertex* node, Point &origin)
{
	...
	FabricVertex *currRowNode = node;
	p1 = node;

	// initialize vector of strands
	std::vector<Strand> strands;

	while (currRowNode != nullptr)
	{
		// make sure currNode is at the leftmost edge
		while (currRowNode->negWarp != nullptr)
		{
			currRowNode = currRowNode->negWarp;
			p1 = currRowNode;
		}
		...
		while (p1 != nullptr)
		{
			...
			else if (error == 2 || error == 3) 
			{
				...
				p1 = p1->posWarp;
				currRowNode = incrementCurrNodeTest ? currRowNode->posWarp : currRowNode;
			}
			else
				p1 = nullptr; // shouldn't happen
		}
		currRowNode = currRowNode->posWeft;
		p1 = currRowNode;
	}
	return strands;
}
